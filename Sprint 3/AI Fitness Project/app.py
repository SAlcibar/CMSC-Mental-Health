...
def home():
    ...
    fitness_goals_list = fitness_goals.split(",") if fitness_goals else []

    # Retrieve fitness experience
    fitness_experience = current_user.fitness_experience
    fitness_experience_list = fitness_experience.split(",") if fitness_experience else []

    # Retrieve workout time
    workout_time = current_user.workout_time
    workout_time_list = workout_time.split(",") if workout_time else []

    # Retrieve dietary preferences
    dietary_preferences = current_user.dietary_preferences
    dietary_preferences_list = dietary_preferences.split(",") if dietary_preferences else []

    # Retrieve fitness challenges
    fitness_challenges = current_user.fitness_challenges
    fitness_challenges_list = fitness_challenges.split(",") if fitness_challenges else []
...
def survey():
    ...
    if request.method == 'GET':
        if current_user.fitness_goals:
            form.fitness_goals.data = current_user.fitness_goals.split(",")  # Split into a list of full strings
    ...
    if form.validate_on_submit():
        current_user.fitness_goals = ",".join(form.fitness_goals.data)  # Join the list into a comma-separated string
